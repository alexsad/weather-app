    ...
    actions:{
        updateCurrLocation(city: string, country: string){
            ...
            const requestForecast = async ({lat, lon}: {lat: number; lon: number}) => {
                const {hourly}: {hourly: weather.Hourly[]} = await fetch(`/weather-api/onecall?units=metric&lat=${lat}&lon=${lon}&exclude=daily,minutely,current`).then(rs => rs.json());
              ...
              const fillWeather = async () => {
                const {main, sys, wind, weather, coord}: weather.WeatherInfo = await fetch(`/weather-api/weather?units=metric&q=${city},${country}`).then(rs => rs.json());
